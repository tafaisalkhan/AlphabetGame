    ...
    create() {
       ...
       //this.game.add.text(30, this.game.world.centerY + 200, 'Touch To Start', style);
        ...
        if(this.message){
            ...
            temp.tint = this.fontColors[1];
            var reStartText =  this.game.add.bitmapText(30, 450 , 'stack', 'Touch To Re-Start', 30);
            reStartText.inputEnabled = true;
            reStartText.events.onInputDown.add(function(){
                //this.state.start('Add')
               var audo = this.game.add.audio('tap');
                audo.play();
               this.state.start('AlphabetGame');
            }, this)
           
        }
        else{
            var startText = this.game.add.bitmapText(10, 250 , 'stack', 'Touch To Start', 40);
